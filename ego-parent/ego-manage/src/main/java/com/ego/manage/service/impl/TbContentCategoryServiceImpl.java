...
public class TbContentCategoryServiceImpl implements TbContentCategoryService{
	...
	public EgoResult rename(TbContentCategory cate) {
		...
		int index = tbContentCategoryDubboServiceImpl.updCategoryById(cate);
		if(index>0){
			egoResult.setStatus(200);
		}
		return egoResult;
	...
	public EgoResult delete(TbContentCategory cate) {
		...
		int index = tbContentCategoryDubboServiceImpl.updCategoryById(cate);
		if(index>0){
			List<TbContentCategory> list = tbContentCategoryDubboServiceImpl.selByPid(cate.getId());
			if(list==null||list.size()==0){
				...
				int i = tbContentCategoryDubboServiceImpl.updCategoryById(category2);
				if(i>0){
					egoResult.setStatus(200);
				}
			}else{
